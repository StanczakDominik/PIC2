...
def test_raised_sin_field_interpolation():
    def sin(x, a, b,c,d):
        return d+a*np.sin(b*x+c)
    field=np.sin(2*np.pi*grid/L)+5
    ...
    a, b, c, d = params
    da, db, dc, dd = np.sqrt(np.diag(covs**2))
    total_rel_error = np.sqrt((da/a)**2+(db/b)**2+(dc/c)**2+(dd/d)**2)
